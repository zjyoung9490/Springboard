...
df <- data.frame(matrix(unlist(VEGFA), nrow = 1, byrow = T), stringsAsFactors = FALSE) 
...
#Creating amino acid position column
final_df$AA_position <- 1:1108

#Creating Hydropathic Index column
final_df$Index <- rollapplyr(final_df$`Hydrophobic Value`, 17, mean, partial=TRUE)

#Summary Statistics

#Selecting only hydrophobic values
aa_vector <- table(final_df$`Amino Acid`)
unlist(aa_vector)
hydrophobic <- aa_vector[c(2, 6, 11, 12, 14, 15, 22)]

#Average hydrophobic value of entire protein
x <- mean(final_df$`Hydrophobic Value`, na.rm = TRUE)

#Percentage of each hydrophobic amino acid in protein
y <- (hydrophobic/1108) *100

#Percentage of all hydrophobic amino acids in protein
z <- ((sum(hydrophobic))/1108) *100

#Creating summary data frame for protein
A <- data.frame(x, y, z) 
A <- spread(A, Var1, Freq)
A["Type"] <- "Non-membrane"
A <- A[colnames(A)[c(10, 1:9)]]
colnames(A)[2] <- "Average_Hydrophobic"
colnames(A)[3] <- "Percentage_Hydrophobic"
row.names(A)[1] <- "VEGFA"

write.csv(A, file = "VEGFA_df")

#Joining individual data frames one final data frame
protein_comparison_df <- full_join(protein_comparison_df, VEGFA_df)